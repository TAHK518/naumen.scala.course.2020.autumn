...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String = x match {
        case x: Boolean => boolFormatter(x)
        case _=> x.toString
    }

    def prettyBooleanFormatter2(x: Any): String = {
        if (x.getClass.getSimpleName.toLowerCase.equals("boolean")) {
            return boolFormatter(x.asInstanceOf[Boolean])
        }
        x.toString
    }
    ...
    def boolFormatter(x: Boolean): String = {
        if (x)
            "правда"
        else
            "ложь"
